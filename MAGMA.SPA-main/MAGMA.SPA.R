MAGMA.SPA <- function(dummyVar="",env=.GlobalEnv) {
	    ...
	    maxP=0.05
			...
			for (i in 1:nModules){
				...
				if (!is.na(NESpVal_module)){
					statmodP <- statmod::permp(sum(abs(NES_perm) >= abs(NES_module)),nperm,length(SNP_data[[1]]),length(geneList[[i]]))
				}else{
					statmodP <- "NA"
				}
				if (i == 1){
	...
	#		write.table(all_output,file=paste("./",outFilePrefix,"MAGMA-SPA-",thisMAGMAinputFile,".txt",sep=""), na="", row.names=T,col.names=NA,sep="\t", quote=FALSE)
		...
		names(all_output)<-names(NES_module_all) <- MAGMAinputs <- c(lapply(statOutList,function(x){x[[3]]})) # AFP edited from do.call(c,__) to c(__)
		...
		my.xlsx=xlsx::createWorkbook()
		for (sheetName in MAGMAinputs) xlsx::addDataFrame(as.data.frame(all_output[[sheetName]]), sheet=xlsx::createSheet(my.xlsx, sheetName), startColumn=1, row.names=TRUE,col.names=TRUE)
		xlsx::saveWorkbook(my.xlsx, paste0("./",outFilePrefix,"MAGMA-SPA",outFileSuffix,".xlsx"))
